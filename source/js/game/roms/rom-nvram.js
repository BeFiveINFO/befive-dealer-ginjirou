...
Rom.nvram = {
	...
	init: function() {
		 ...
		 */
		 if(Register.free_play) {
			...
			Register.replenishmentTime = PCB.nvramController.cookie_read('replenishmentTime',PCB.clock.unixTime() + Register.replenishment_every,712);
		 ...
		 */
		 ...
		 Register.user_id = PCB.nvramController.cookie_read('userID_DG','');
	},
	/**
	...
	readMuteState: function() {
		var _soundMuteState = PCB.nvramController.cookie_read('soundMuteState',Register.mute_sound);
		if(_soundMuteState == 'false') {
			return false;
		} else {
			return true;
		}
	...
	/**
	* Updates the current user id. Key is 'userID_DG'
	*
	* @param      string user id to be saved
	*/
	updateUserID: function( _userID ) {
		PCB.nvramController.cookie_write('userID_DG',String(_userID));
	},
	/**
...
/*
'replenishment_credits': 200,
'replenishment_max': 1000,
*/